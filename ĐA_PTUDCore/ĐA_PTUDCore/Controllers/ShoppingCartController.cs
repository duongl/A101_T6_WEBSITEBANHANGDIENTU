...
namespace ShopBanLaptop2.Controllers
{
    public class ShoppingCartController : Controller
    {
        ...
        {
            var shopCart = JsonConvert.DeserializeObject<List<SCartInfo>>(ctx.HttpContext.Session.GetString("shopCart"));
            int a = shopCart.Where(r => r.lap.Id == id).FirstOrDefault().soLuong;
            int b = LaptopConnect.Instance.searchById(id)[0].Quantity;
            if (a >= b)
            {
                ctx.HttpContext.Session.SetInt32("overload", 1);
                return RedirectToAction("showShopCart");
            }
            else
            {
                ctx.HttpContext.Session.SetInt32("overload", 0);
        ...
        public ActionResult thanhToan()
        {
            ...
            if(kq > 0)
            {
                ...
                foreach (var item in shopCart)
                {
                    ...
                    LaptopConnect.Instance.decreaseQuantity(item.lap.Id, item.soLuong);
                }
