...
namespace ShopBanLaptop2.Models
{
    public class LaptopConnect
    {
        ...
        public int EditLaptop(int id,string Name,int Quantity, int idCategory, double price, string productCompany, double monitor, string CPU, string RAM, string Image, string GPU, string HardDisk)
        {
            int kq = -1;
            //=============== Kiem tra trung ten =======================
            string query = "select COUNT(*) from Laptop where id = '" + id + "'";
            ...
            if (kt > 0)
            {
                ...
                query2 += "set  Name = N'"+ Name +"', quantity = "+Quantity+", idCategory = "+ idCategory + ", price="+ price + ",productCompany = N'"+productCompany+"', monitor="+ monitor+", CPU = N'"+CPU+"' ,RAM = N'"+RAM+"', Image = N'"+Image+"', GPU = N'"+GPU+"', HardDisk = N'"+HardDisk+"'";
                query2 += "where id = " + id;
                kq = DataProvider.Instance.ExecuteNonQuery(query2);
            }
            return kq;
        ...
        public int decreaseQuantity(int id, int quantity)
        {
            int kq = -1;
            //=============== Kiem tra trung ten =======================
            string queryLap = "select * from Laptop where id = '" + id + "'";
            List<Laptop> lap = getListLaptops(queryLap);
            // ============== Nếu tồn tại tên trùng ====================
            if (lap.Count > 0)
            {
                string query2 = "update Laptop ";
                query2 += "set quantity = " + (lap[0].Quantity - quantity);
                query2 += "where id = " + id;
                kq = DataProvider.Instance.ExecuteNonQuery(query2);
            }
            return kq;
